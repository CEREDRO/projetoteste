define([
	'text!servicos/risco/visao/formulario.html',
	'bootstrap',
	'ajaxStatus',
	...
	'servicos/risco/controle/AvaliarControle',
...
], function(template, Bootstrap, AjaxStatus,FormularioModelo,FormularioColecao,PermissaoModelo,OcorrenciaControle,QuadroControle,RelacionamentoControle,AjudaControle,AjudaModelo,ItemQuadroControle,AvaliarControle,ItemQuadroModelo,ItemDemonstrativoModelo,ItemDemonstrativoColecao,GradacaoOcorrenciaModelo,GradacaoOcorrenciaColecao,EstruturaModelo,ContaModelo) {
	var FormularioControle = Backbone.View.extend({
		...
		initialize: function() {
			...
			this.model = new FormularioModelo();
			...
			this.collection.fetch({
				success: function(collection, response) {
	            	...
	            	collection.each(function(data){
	            		...
	            		if(data.get('descricao2') != null && data.get('descricao2') != ""){
	            			if(data.get('descricao2').trim() == "Clique em adicionar para responder." && data.get('obrigatorio') == 0){
		            			exibeBtnAvaliar = 1;
		            		}
	            		}
	            	});
	            	...
	            	if (exibeBtnAvaliar == 0){
	            		var ajuda = new AjudaModelo();
	            		ajuda.url = "rest/risco/controleavaliacao/validaavaliacao/" + self.pessoa.get('codigo') + "," + self.modelo;
	            		ajuda.fetch({
	            			success: function(ajuda, response) {
	            				if(ajuda.get('descricao') == "OK"){
	            					self.avaliar = new AvaliarControle();
	            					self.avaliar.pessoa = self.pessoa;
	            					self.avaliar.modelo = self.modelo;
	        	            		$('#divAvaliar').html(self.avaliar.render().el);
	        	            		self.avaliar.carregaAvaliacaoControle();
	            				}else{
									$('#divAvaliar').html('');
								}
	            			},
		...
		acoes: function(e){
			...
			if (acao == "A"){
				...
				if (tipo == "O"){
					...
					if (acao == "U"){
						var perguntaqv = $(e.target).data('pergunta');
						this.alteraQuadro(codigoQuadro,perguntaqv,$(e.target).data('inicio'),$(e.target).data('fim'));
					}else if (acao == "D"){
					...
					if (acao == "U" || acao == "D"){
						...
						if (acao == "I"){
							...
							permissao.fetch({
								success: function(permissao, response) {
									if (permissao.get('codigo') == 1){
										self.insereRelacionamento($(e.target).data('codigo'),self.pessoa.get('codigo'),inicio,fim,valor,codigoNovo);
									}else{
		...
		insereQuadro: function(codigoPessoa,codigoModelo,inicio,fim,descricao){
			...
			if(inicio == 1 && fim == 1){
				this.quadro.model.set('codigo',0);
				this.quadro.model.set('tipo',0);
				this.quadro.perciste();
			}else{
				$('#divQuadro').html(this.quadro.render().el);
				this.quadro.carregaDados();
			}
		},
		alteraQuadro: function(codigo,pergunta,inicio,fim){
			this.quadro = new QuadroControle();
			this.quadro.acao = "A";
			this.quadro.inicio = inicio;
			this.quadro.fim = fim;
			this.quadro.pergunta = pergunta;
		...
		insereRelacionamento: function(pergunta,codigoPessoa,inicio,fim,valor,subPergunta){
			...
			this.relacionamento.subPergunta = subPergunta;
		...
		concluido: function(tipo,valor,novo){
			...
			this.stopListening();
			this.montaFormulario(this.pessoa, this.modelo);
		},
					...
					success: function(model, response) {
						...
						if (model.get('listaItemDemonstrativo').length > 0){
							...
							for (var i = 0; i < model.get('listaItemDemonstrativo').length; i++){
								...
								if (model.get('listaItemDemonstrativo')[i].estrutura.conta.gradacaoOcorrencia != null && model.get('listaItemDemonstrativo')[i].estrutura.conta.gradacaoOcorrencia.tipoOcorrencia != 0){
		...
		carregarRLink: function(e){
			reiniciaContadorSessao();
			if($(e.target).data('codigo') == '56621'){
				alert('O CNPJ da Caixa não deve ser pesquisado.');
				return;
			}
			//alert(window.location.href);
			window.open(window.location.href + "?pessoa=" + this.pessoa.get('codigo') + "&modelo=" + $(e.target).data('pergunta') + "&relacionado=" + $(e.target).data('codigo'));
			/*
			var url = window.location.href + "?pessoa=" + $(e.target).data('codigo') + "&modelo=" + $(e.target).data('pergunta');
